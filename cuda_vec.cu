...
__device__ __forceinline__ void add_v(Vec3 *vec, float v) {
  vec->x += v;
  vec->y += v;
  vec->z += v;
}
...
__device__ __forceinline__ void sub_v(Vec3 *vec, float v) {
  vec->x -= v;
  vec->y -= v;
  vec->z -= v;
}
...
__device__ __forceinline__ void mult_v(Vec3 *vec, float v) {
  vec->x *= v;
  vec->y *= v;
  vec->z *= v;
}
__device__ __forceinline__ void divide_v(Vec3 *vec, float v) {
  vec->x /= v;
  vec->y /= v;
  vec->z /= v;
}
__device__ __forceinline__ float dot_v(Vec3 *vec, Vec3 *target) {
...
__device__ __forceinline__ float sq_len_v(Vec3 *vec) {
  return vec->x * vec->x + vec->y * vec->y + vec->z * vec->z;
}
__device__ __forceinline__ float len_v(Vec3 *vec) {
...
__device__ __forceinline__ void rotateX(Vec3 *dir, float angle) {
...
__device__ __forceinline__ void rotateY(Vec3 *dir, float angle) {
  float cosA = cos(angle * M_PI / 180.0);
  float sinA = sin(angle * M_PI / 180.0);
...
__device__ __forceinline__ void rotateZ(Vec3 *dir, float angle) {
  float cosA = cos(angle * M_PI / 180.0);
  float sinA = sin(angle * M_PI / 180.0);
...
__device__ __forceinline__ void rotateDirection(Vec3 *dir, float angleX,

...
__host__ __device__ __forceinline__ unsigned my_rand(unsigned *seed_p) {
...
__host__ __device__ __forceinline__ float my_drand(unsigned *seed_p) {
...
__host__ __device__ __forceinline__ int ray_intersect(Vec3 *o, Vec3 *d,
                                             const Object *object,
...
__host__ __device__ __forceinline__ int find_closest_hit(Vec3 *o, Vec3 *d, const Object *objects, int count,
...
__host__ __device__ __forceinline__ float random_normal(unsigned *seed) {
...
__host__ __device__ __forceinline__ void random_direction_hemi_and_lerp(Vec3 *target,
                                                               Vec3 *normal,
                                                               unsigned *seed,
                                                               float lerp) {
...
__host__ __device__ __forceinline__ void move_point_randomly_in_circle(Vec3 *target, unsigned *seed, float radius) 
{
  float theta = 2 * 3.1415926 * my_drand(seed);
...
__host__ __device__ __forceinline__ float lerp(float a, float b, float f) 
{
  return a * (1.0 - f) + (b * f);
}